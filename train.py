...
def get_best_model_version(model_registry_name, key="status", value="best"):
    ...
    model_versions = client.search_model_versions(filter_string=f"name='{model_registry_name}'")
...
def compare_and_update_models(accuracy, model_registry_name):
    ...
    latest_version = mlflow.register_model(f"runs:/{mlflow.active_run().info.run_id}/model", model_registry_name)
    
    # Compare accuracies
    if best_model_accuracy is None or accuracy > best_model_accuracy:
        if best_model_version is not None:
            client.set_model_version_tag(model_registry_name, best_model_version.version, "status", "previous_best")
        client.set_model_version_tag(model_registry_name, latest_version.version, "status", "best")
        print(f"New model with accuracy {accuracy:.2f} is now the best model.")
    else:
        print(f"New model with accuracy {accuracy:.2f} did not outperform the current best model with accuracy {best_model_accuracy:.2f}.")
